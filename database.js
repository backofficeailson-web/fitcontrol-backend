...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS fotos_posturais (
    ...
    aluno_id INTEGER NOT NULL,
    tipo TEXT NOT NULL,
    -- base64 provisório; migrar para Cloudinary/S3 em produção
...
// ============================================
// MIGRAÇÕES SEGURAS (colunas da tabela alunos)
// ============================================
const migrations = [
...
for (const m of migrations) {
  try {
    ...
    if (!e.message.includes('duplicate column name')) {
      console.log(`Migração ignorada: ${m.col} → ${e.message}`);
    }
  }
}

// Apenas em desenvolvimento: atribuir user_id padrão para alunos sem dono
try {
  const defaultUser = db.prepare('SELECT id FROM users LIMIT 1').get();
  if (defaultUser) {
    db.prepare('UPDATE alunos SET user_id = ? WHERE user_id IS NULL').run(defaultUser.id);
  }
} catch (e) {}
